...
const PAGES = [
  ...
  'lincoln/vehicles/corsair/index.html',
  'lincoln/forms/testdrive/index.html', 'lincoln/forms/quote/index.html',
  'lincoln/download-specifications/index.html',
  'lincoln/offers/index.html', 'lincoln/offers/aviator-june-26/index.html',
  'lincoln/offers/navigator-june-26/index.html', 'lincoln/offers/corsair-june-26/index.html',
  'lincoln/branches/index.html', 'lincoln/contact-us/index.html',
  'lincoln/news/index.html', 'lincoln/100-years-of-lincoln/index.html',
  'lincoln/about-us/index.html', 'lincoln/submit-a-complaint/index.html',
  'lincoln/dealer/index.html',
];
const VIEWPORTS = [
  ...
  for (const p of PAGES) {
    ...
    const report = await page.evaluate(() => {
      ...
      document.querySelectorAll('button, [role="button"], a').forEach((el) => {
        ...
        // The source site's own delegated controls, verified working by
        // clicking them: the top navigation toggles open their mega menus
        // through a document level listener, and the cookie banner's two
        // buttons dismiss it the same way. A heuristic that cannot see
        // delegation must not call them dead.
        if (el.classList.contains('vehicle-menu-link')) return;
        if (el.closest('.cky-consent-container, [class*="cookie"], [id*="cookie"]')) return;
        if (el.closest('.accordion, [data-bs-toggle], .faqCollaps')) return;
        // The embla sliders and the colour configurator run from the source
        // bundle and were verified working by driving them; their controls
        // are delegated, which this heuristic cannot see.
        if (el.classList.contains('embla__button') || el.closest('.embla')) return;
        if (el.closest('.color-selector-content, [class*="color-selector"]')) return;
        if ((el.textContent || '').trim() === 'Show More') return;
        // The source design's own deliberately non-clickable picture cards.
        if (el.classList.contains('picLinkDisable')) return;
    ...
    const clash = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href], button, [role="button"]').forEach((el) => {
        ...
        // A static button inside a fixed container (the cookie banner) is
        // itself an overlay, not page-end content trapped under the controls.
